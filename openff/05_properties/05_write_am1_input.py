...
def write_server_config(outdir, port=8001):
    config = {"backend_config": {"type": "dask-local",
                                "number_of_workers": 1,
                                "resources_per_worker": {"n_processes": 12,
                                                         "n_gpus": 1,},
                                },
            "port": port,
            "working_directory": str(outdir / "evaluator_working-data"),
...
def write_prop_input(outdir, training_set):
...
def write_forcefield(outdir, training_set, charge_dataframes):
...
def write_opt_json(outdir, ff, parameters):
    ...
    dct = {
        "id":  "openeye-am1bcc",  # only accepts a-z0-9 and -
        "study_id": "charge-models-am1bcc",
        "project_id": "charge-models",
        "name": f"AM1BCC, OpenEye",
        "description": "AM1BCC",
...
def write_input(given_training_set, outname="05_vdw"):
    outdir = pathlib.Path(outname) / "openeye"
    ...
    ff, parameters = write_forcefield(outdir, found_training_set, dfs)
    write_prop_input(outdir, found_training_set)
    write_opt_in(outdir)
    write_server_config(outdir)
    write_opt_json(outdir, ff, parameters)
