...
if __name__ == "__main__":
    ...
    optmol = qcel.models.Molecule.from_file(args.infile)
    confid = path.stem.split("_")[1][1:]
    state = path.stem.split("_")[2]
    # confid = path.stem.split("_opt")[0].split("_c")[1]
    ...
    response_file = path.parent / f"03_c{confid}_{state}_response_ids.json"
    ...
    computation = dict(
        program="psi4",
        basis="cc-pV(D+d)Z", #result.qc_spec.basis,
        method="PW6B95", #result.qc_spec.method,
        driver="energy",
    ...
    response_ids = list(response.ids)

    with open(response_file, "w") as f:
        json.dump(response_ids, f)
    print(f"Wrote to {response_file}")
    
    results = wait_for_results(client, response_ids)
